...
def find_affiliations(not_rest_affs, src):
    ...
    for entry in all_entries:
        ...
        if affiliation_name not in affiliations and affiliation_name != "" and check_affs(affiliation_name,
                                                                                          not_rest_affs):  # and not check_for_dublicates(affiliation_name,affiliations):
...
def central_page(request):  # ONLY CENTRAL PAGE EDITION
    ...
    return render(request, 'index_central.html',
                  {'total_publications': total_publications, 'total_citations': total_citations,
                   'total_countries': total_countries, 'affiliations': all_affiliations,
                   'subject_areas': len(all_subject_araes), 'departements': departements})
...
def home(request):
    ...
    return render(request, 'index.html', {'total_publications': total_publications, 'total_citations': total_citations,
                                          'total_countries': len(total_countries), 'affiliations': all_affiliations,
                                          'keywords': total_keywords, 'subject_areas': all_subject_areas,
...
def home_search_by_keyword(request):
    ...
    try:
        ...
        return render(request, 'publications_search_results.html',
                      {'results': [], 'keyword': "", 'home_search_by_keyword': True})
    keywordID = k.id
    ...
    return render(request, 'publications_search_results.html',
                  {'results': sorted_data, 'keyword': keyw.strip(), 'home_search_by_keyword': True})
...
def publications(request):
    ...
    return render(request, 'publications.html',
                  {'top_publications': top_publications[0:5], 'keywords': all_keywords, 'top_keywords': top_keywords,
                   'authors': all_authors, 'max_citations': top_publications[0].pub_citations})
...
def publications_search(request):
    if request.method == "GET":
        ...
        data = sorted(results, key=lambda k: k['date'], reverse=True)
        return render(request, 'publications_search_results.html',
                      {'results': data, 'all_keywords': all_keywords, 'doc_title': doc_title,
                       'conference': conference, 'journal': journal, 'book': book, 'other': other,
                       'max_citations': max_citations, 'ex_year_from': ex_year_from,
...
def publications_keyword(request, keyWord):
    ...
    sorted_data = sorted(data_to_return, key=lambda k: k['date'], reverse=True)
    return render(request, 'publications_search_results.html',
                  {'results': sorted_data, 'keyword': keyw.strip(), 'top_keyw_search': True})
...
def author_publications_keyword(request, authorName, keyWord):
    ...
    for pub in author_pubs:
        ...
        all_entries = keyword_publication.objects.filter(
            keyword_id=keywordID)  # all the keywords which used in this publication
        for entry in all_entries:
    ...
    sorted_data = sorted(data_to_return, key=lambda k: k['date'], reverse=True)
    return render(request, 'publications_search_results.html',
                  {'results': sorted_data, 'keyword': keyw.strip(), 'author_name': author_name})
    ...
    if author_exist:
        ...
        for entry in all_entries:  # loop through all author's publications
            ...
            for a in all_co_authors:
                ...
                try:
                    co_author_aff = co_author_affiliation.objects.filter(co_author_id=co_author_id)[
                                    :1].get()  # in case of many results
        ...
        return render(request, 'author.html', {'author_name': authorName, 'author_url': author_url, 'h_index': h_index,
                                               'citation_impact': citation_impact,
                                               'author_subject_areas': author_subject_areas,
                                               'total_publications': len(all_entries),
                                               'total_citations': total_citations
            , 'co_authors': co_authors, 'has_photo_profile': authorName.lower().replace(" ", "_"),
                                               'author_publications': sorted_by_date, "orcID": author_orcID})
...
def access_db(request):
    if request.method == "GET":
        if request.GET['action'] == "access_db_publications":
            ...
            not_rest_affs = ["Thessaly", "thessaly", "volos", "Volos"]  # must be dynamic
            return HttpResponse(json.dumps(find_affiliations(not_rest_affs, "access_db")),
